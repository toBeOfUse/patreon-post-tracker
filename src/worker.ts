...
const sortableColumns = ['comment_count', 'like_count', 'published_at'];
const perPage = 20;
...
export class PostStorage extends DurableObject<Env> {
	...
	storeRunEnd(startTime: Date, duration: number, retrieved: number) {
	...
	upsertJsonPosts(posts: PatreonApiResponse) {
	...
	async getPosts(page = 1, sortBy = 'comment_count', sortDirection = 'desc', query = '') {
		if (!sortableColumns.includes(sortBy) || !['asc', 'desc'].includes(sortDirection)) {
...
export default {
	...
	async scheduled(controller, env, ctx) {
		...
		const startTime = await object.storeRunStart();
		...
		while (retrievedPosts < 100) {
			...
			await object.upsertJsonPosts(posts);
		...
		console.log('done');
		await object.storeRunEnd(startTime, (Date.now() - startTime.getTime()) / 1000, retrievedPosts);
	},
